...
deploy_qemu:
	qemu-system-avr -machine mega2560 -bios blink_led.elf -nographic -serial telnet:localhost:5678,server=on,wait=off
	# In another shell: telnet localhost 5678
	# Quit qemu: (qemu) quit
	# Quit telnet: Ctrl + ] -> telnet> quit

test_qemu:
	qemu-system-avr -machine mega2560 -bios blink_led.elf -nographic -serial telnet:localhost:5678,server=on,wait=off -S -s
